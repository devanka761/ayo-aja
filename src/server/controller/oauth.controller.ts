...
const HOST: string = isProd ? "https://kirimin.devanka.id" : "http://localhost:7000";
  ...
  async discord(code:string): Promise<PayloadData> {
    const getAccess = await fetch("https://discord.com/api/v10/oauth2/token", {
      ...
      "body": new URLSearchParams({
        "client_id": cfg.DISCORD_CLIENT_ID as string,
        "client_secret": cfg.DISCORD_CLIENT_SECRET as string,
        "grant_type": "authorization_code",
        "code": code,
