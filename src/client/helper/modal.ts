import { KiriminHttpResponse, ModalAlert, ModalConfirm, ModalPrompt, ModalSelect } from "../types/helper.type";
...
const modal = {
  ...
  async loading(newfunc: Promise<object|string|number|void>, msg:string|null|undefined = "LOADING"): Promise<KiriminHttpResponse|object> {
  ...
  select(options: object): Promise<string|null> {
    return new Promise(resolve => {
      ...
      s.items.forEach(itm => {
        ...
        radio.innerHTML = `
        <label for="${optionId}-${itm.id}">
          <input type="radio" name="${optionId}" id="${optionId}-${itm.id}" value="${itm.id}" required />
          <p>${itm.label}</p>
        </label>`;
        const radioInp = radio.querySelector('input');
        if(itm.activated && radioInp) {
