...
import { KiriminHttpResponse, LangObject, Languages } from "../types/helper.type";
...
const ProviderObject = {
  USE_OAUTH_DISCORD: "discord" as string,
  USE_OAUTH_GITHUB: "github" as string,
  USE_OAUTH_GOOGLE: "google" as string
}

function loginProvider(provider:LoginProvider): HTMLElement {
  const btn: HTMLElement = document.createElement("div") as HTMLElement;
...
class SignEmail {
  ...
  formListener():void {
    ...
    Object.keys(config).filter(k => k.includes("USE_OAUTH") && config[k] === true).forEach(k => {
